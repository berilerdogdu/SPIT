...
def get_stable_dtu(num_of_infReps, infReps_dtu_genes, infReps_flagged_genes, wrst_pos_genes, flagged_genes):
    ...
    dtu_genes_stable = [g for g, count in dtu_genes_counts.items() if count > int(num_of_infReps*0.75)]
    flagged_gene_counts = Counter(infReps_flagged_genes)
    flagged_genes_stable = [g for g, count in flagged_gene_counts.items() if count > int(num_of_infReps*0.75)]
...
def main(args):
    ...
    genotype_cluster_df.to_csv(os.path.join(args.O, "SPIT_analysis", "spit_cluster_matrix.txt"), sep = '\t')
